...
A free, open-source desktop office suite. Nine apps in one download, working
...
cd office
npm install
npm test          # 893 tests, no network needed
...
packages/
  ...
  calendar         iCalendar read and written, recurrence, time zones
  contacts         vCard and CSV address books, read and written
...
npm test                # the engine suite — 893 checks, no windows
...
`npm run gate` is what runs before a release: 893 engine tests, 12 editing checks,
434 application checks — most of them pressing the real ribbon buttons and reading what the page paints, a block of them driving the actual mouse and keyboard — and 10 window captures. The windows sit off the desktop, so a run never gets in your way and you cannot close it by mistake; it takes about two and a half minutes.
