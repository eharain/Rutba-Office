...
cd office
npm install
npm test          # 782 tests, no network needed
...
npm test                # the engine suite — 782 checks, no windows
...
`npm run gate` is what runs before a release: 782 engine tests, 9 editing checks,
313 application checks — most of them pressing the real ribbon buttons and reading what the page paints, a block of them driving the actual mouse and keyboard — and 10 window captures. The windows sit off the desktop, so a run never gets in your way and you cannot close it by mistake; it takes about two and a half minutes.
