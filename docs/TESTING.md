...
`RUTBA_VERIFY_ONLY=pages,grips,panes,float,polish,shapes,bullets,clip,footer,find,sections,hidden,background,table,fill,pics,look,dropcap,effects,bookmarks,ruler,columns,update,viewer,slideshow,links,home,freeze,errors,zoom,fit,cards,sheetpic,providers npm run verify:apps` runs only the
