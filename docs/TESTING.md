...
`RUTBA_VERIFY_ONLY=pages,grips,panes,float,polish,shapes,bullets,clip,footer,find,deckfind,sections,hidden,background,table,chart,fill,pics,look,dropcap,effects,bookmarks,xref,captions,ruler,columns,update,viewer,slideshow,links,home,recent,freeze,errors,watch,sparklines,zoom,fit,cards,sheetpic,providers,signature npm run verify:apps` runs only the
