...
The gap list ([GAPS.md](../GAPS.md)) was written from the code the same day,
...
- **Printing.** Nothing here could print: Word's button called a command
  ...
  every window has Ctrl+P and a dialog that says how many pages before it
  prints anything — and how much it had to shrink them — with the printers
  this machine can reach, copies, paper, orientation and margins. Where the
  pages are cut is worked out beside the engines and tested there; the page
  itself is Chromium's. Worksheets adds the sheet or the workbook,
  fit-to-width or fit-to-page, the print area, repeated rows, gridlines and
  headings; Presentation adds full-page slides, notes pages, or a handout of
  one to nine. Export as PDF goes through the same door for all three, so a
  workbook and a deck can be exported at all, and what is printed and what
  is exported cannot drift apart.
- **Page setup lives in the workbook.** Paper, orientation, margins,
  scaling, the print area, the repeated rows, gridlines, headings and
  centring are read from and written to the file where Excel keeps them, so
  they reach whoever opens it next. Writing them right needed the sheet's
  tail written in the order the schema gives, and print areas matched by
  sheet as well as by name.
- **Rich text, written.** The installer had claimed the suite edits `.rtf`
  while Ctrl+S refused. Words, font, size, weight, slant, underline, strike,
  colour, alignment, list level and tables go out and come back.
- **Unsaved work outlives a crash.** A changed document is copied into the
  profile — every half minute, or every few minutes for a workbook that
  takes seconds to serialise, since the interval follows what the last copy
  cost — and the launcher offers what a crash took back by name, with the
  time and where it belonged. Saving or closing removes the copy.
- **The formula language:** array constants (`{1,2;3,4}`), `LET`, and a
  reference to an empty cell that is nothing rather than a zero — `COUNT`
  of a blank is 0 and `AVERAGE` of one is `#DIV/0!`, as in Excel.
- **Worksheets' Enter commits what was typed**, not what the last render
  saw: a keystroke sixty milliseconds before Enter used to be lost.

## What reviewing this release found

The work above was read back through, run, and then run against files made
to be awkward. What that found is fixed here too.

Two of them stopped the release outright. Window creation used Electron's
`app` object without importing it, and the reference sat outside the try
that guards it, so every window but the launcher threw: the first gate run
over this work passed 3 of its 35 blocks. And the association list carries
the owning app so the installer can give each type that app's icon —
electron-builder validates every association against its own schema and
refuses a property it does not know, so `npm run dist` could not start.

The rest were older than this release:

- **Export as PDF threw, every time.** The document PDF writer answers an
  object — the buffer and the page count — and the service handed the object
  itself to `Buffer.from`. The unit tests destructure the answer, so they
  never saw it, and no window check exports. Worksheets and Presentation have
  no PDF writer at all; their buttons said the export "is done from the
  window", and nothing in any window does it. They now say what they can
  write instead.
- **The HTML export wrote the document's own characters as markup.** A
  paragraph reading "5 < 6 & 7 > 4" came back as "5   4", and a document
  containing a script tag produced a page that runs it. The reader was the
  other half: one replace of every tag with a space, so a page arrived as a
  single paragraph with its entities spelled out and its stylesheet in the
  text.
- **A range past the last used cell answered with the last row that holds
  something.** Ranges are clipped to the used area so a whole-column
  reference does not walk a million rows; the clip pulled the end of a range
  back before its start, and the rule that normalises a backwards range then
  landed it on the boundary. On a sheet ending at row 5, =SUM(D7:D9) returned
  D5. Wrong numbers, quietly.
- **A reference to an empty cell counted as a zero.** =COUNT(A1) on a blank
  answered 1, and =AVERAGE(A1) answered 0 where Excel answers #DIV/0!.
- **A UTF-16 text file was refused as an MP3.** FF FE is the mark Notepad
  writes for "Unicode" and PowerShell writes whenever it redirects; it is
  also a valid MPEG frame sync. Every UTF-16 text file on Windows was
  unopenable.
- **A document under a workbook's name hung the window** — thirty seconds of
  blank, then nothing. The window now says which kind it edits, and a file of
  another kind is refused by name in a third of a second.
- **A file the decoder refuses left the media window empty for ever**, and a
  picture that would not decode showed a frame with nothing in it and a toast
  that fades. All three picture and media windows say what happened now.
- **The disk failing reached people as error codes.** Saving over a file Word
  still had open said "EPERM: operation not permitted".
- **The first letter typed into the formula bar was dropped.** Beginning an
  edit is a round trip, and until it returned the bar fell back to the cell's
  old contents — so "hello" arrived as "ello" whenever the machine was busy
  enough. The same race the grid had already been given a flag for.
- **A workbook with no sheets, and a relationship with no target**, each
  threw from inside an engine rather than being refused: found by
  `tools/fuzz-open.js`, which damages good files at random and opens them
  until something answers with anything other than a sentence. Four and a
  half thousand damaged files now do.

And the checks that missed them: the corpus run counted a picture that never
decoded as shown; the editing run used the developer's own profile; the smoke
run photographed the machine's real pictures folder; a check run that threw
before its first window hung for ever instead of failing; and all three window
harnesses read a console-message shape Electron has deprecated, so the day it
is dropped they would go quietly blind.

## Verified

- 654 engine tests.
- 145 application checks, off the desktop, with real mouse and keyboard input.
