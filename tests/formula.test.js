...
test('the day that never existed: serial 60 is 29 February 1900, as Excel has it', () => {
  const wb = new Spreadsheet();
  wb.addSheet('S');
  const set = (row, input) => wb.setCell('S', row, 0, input);
  set(0, '=DATE(1900,2,29)');
  set(1, '=DAY(60)');
  set(2, '=MONTH(60)');
  set(3, '=YEAR(60)');
  set(4, '=DATE(1900,3,1)');
  set(5, '=DATE(1900,3,0)');
  set(6, '=DAY(61)');
  set(7, '=TEXT(60,"d mmm yyyy")');
  set(8, '=WEEKDAY(60)');
  set(9, '=WEEKDAY(1)');
  set(10, '=WEEKDAY(45000)');
  set(11, '=WEEKDAY(59)');
  const got = (row) => wb.getValue('S', row, 0);
  assert.equal(got(0), 60, 'DATE(1900,2,29) is the phantom day');
  assert.equal(got(1), 29);
  assert.equal(got(2), 2);
  assert.equal(got(3), 1900);
  assert.equal(got(4), 61, '1 March 1900 follows it');
  assert.equal(got(5), 60, 'day zero of March 1900 is the phantom day too');
  assert.equal(got(6), 1);
  assert.equal(got(7), '29 Feb 1900');
  assert.equal(got(8), 4, 'Excel calls the phantom day a Wednesday');
  assert.equal(got(9), 1, 'Excel calls 1 January 1900 a Sunday, and the calendar is not consulted');
  assert.equal(got(10), 4, '15 March 2023 was a Wednesday, and from March 1900 on the two agree');
  assert.equal(got(11), 3, 'Excel says 28 February 1900 is a Tuesday; the almanac says Wednesday');
});

test('TEXT() speaks every format code the grid does', () => {
  const wb = new Spreadsheet();
  wb.addSheet('S');
  const set = (row, input) => wb.setCell('S', row, 0, input);
  set(0, '=TEXT(45000,"dd/mm/yyyy")');
  set(1, '=TEXT(45000.5,"hh:mm AM/PM")');
  set(2, '=TEXT(0.125,"0.0%")');
  set(3, '=TEXT(1234.5,"#,##0.00")');
  set(4, '=TEXT(-3,"0;(0)")');
  set(5, '=TEXT("abc","0.00")');
  set(6, '=TEXT(45000,"dddd, d mmmm yyyy")');
  set(7, '=TEXT(0.5,"# ?/?")');
  set(8, '=TEXT(1234567,"0.00E+00")');
  set(9, '=TRIM(TEXT(2.75,"# ??/??"))');
  set(10, '=TEXT(1.5,"?/?")');
  set(11, '=TEXT(0.125,"# ?/8")');
  set(12, '=TRIM(TEXT(3,"# ?/?"))');
  set(13, '=TEXT(-0.5,"# ?/?")');
  set(14, '=TEXT(0.000123,"0.0E+0")');
  set(15, '=TEXT(1234567,"##0.0E+0")');
  set(16, '=TEXT(12345678,"##0.0E+0")');
  set(17, '=TEXT(0,"0.00E+00")');
  const got = (row) => wb.getValue('S', row, 0);
  assert.equal(got(0), '15/03/2023');
  assert.equal(got(1), '12:00 PM');
  assert.equal(got(2), '12.5%');
  assert.equal(got(3), '1,234.50');
  assert.equal(got(4), '(3)');
  assert.equal(got(5), 'abc', 'text that is not a number stays text');
  assert.equal(got(6), 'Wednesday, 15 March 2023');
  assert.equal(got(7), ' 1/2', 'the whole part of # disappears and leaves its space, as in Excel');
  assert.equal(got(8), '1.23E+06');
  assert.equal(got(9), '2 3/4');
  assert.equal(got(10), '3/2', 'no whole part: an improper fraction');
  assert.equal(got(11), ' 1/8', 'a fixed denominator');
  assert.equal(got(12), '3', 'a whole number leaves the fraction blank');
  assert.equal(got(13), '- 1/2');
  assert.equal(got(14), '1.2E-4');
  assert.equal(got(15), '1.2E+6', 'three integer placeholders: the exponent is a multiple of three');
  assert.equal(got(16), '12.3E+6');
  assert.equal(got(17), '0.00E+00');
});
