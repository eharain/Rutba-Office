...
import { verifyDeckThemes } from './verify-deck-themes.js';
import { verifyDeckMaster } from './verify-deck-master.js';
...
import { verifyViews } from './verify-views.js';
...
export async function verifyApps({ windows, doc, broadcast = null, update = null }) {
  ...
  /* ── Worksheets: Evaluate Formula, Calculation Options ───────────────── */
  const sheetEvaluate = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyEvaluate({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: Review → threaded comments ───────────────────────────── */
  const sheetComments = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyComments({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: Scatter, Page Break Preview, Split ──────────────────── */
  const sheetViews = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyViews({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: Protect Workbook, Allow Edit Ranges, Hide / Move sheets ─ */
  const sheetProtect = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyProtect({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: Page Layout view, Custom Views, Background ─────────── */
  const sheetLayoutViews = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyLayoutViews({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: PivotChart, Slicers ───────────────────────────────── */
  const sheetSlicers = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
  ...
  /* ── Worksheets: Page Layout → Themes, Colours, Fonts, Effects ─────── */
  const sheetThemes = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifySheetThemes({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };
  ...
  const sheetAnalysis = async () => {
    ...
    await verifyAnalysis({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };

  /* ── Worksheets: Advanced filter, Clear, Flash Fill ──────────────────── */
  const sheetDataTools = async () => {
    const capture = async (win, name) => {
      if (!process.env.RUTBA_VERIFY_CAPTURE) return;
      fs.writeFileSync(path.join(process.env.RUTBA_VERIFY_CAPTURE, name), (await win.webContents.capturePage()).toPNG());
    };
    await verifyDataTools({ open, check, until, wait, press, errorsIn, capture, doc, sessionFor }, { dir });
  };
  ...
  // RUTBA_VERIFY_ONLY=pages,grips,panes,float,polish,shapes,fill,pics,ruler,columns,update,viewer,slideshow,links,home,recent,freeze,errors,watch,sparklines,fit,sections,hidden,background,effects,bookmarks,xref,captions,providers,signature,deckfind,sendlater,ooo,arrange,deckfx,toc,track,outline,datatools,equations,transitions,animations,evaluate,comments,views,mailmerge,labels,themes,master,deckcomments,deckmath,protect,layoutviews,analysis,a11y,spelling,textbox,wordarrange,slicers,encrypted,sheetarrange,hyphen,citations,restrict,wordindex,figures,sheetthemes: those blocks alone, for working on them.
  ...
  if (only.length) {
    ...
    if (only.includes('evaluate')) await sheetEvaluate();
    if (only.includes('comments')) await sheetComments();
    if (only.includes('views')) await sheetViews();
    if (only.includes('protect')) await sheetProtect();
    ...
    if (only.includes('themes')) await verifyDeckThemes({ open, check, until, wait, press, errorsIn, doc, sessionFor }, { dir: path.dirname(files.pptx) });
